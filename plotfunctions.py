...
def color_plot(x=None,y=None,z=None,x_scale=1,y_scale=1,z_scale=1,x_slice=None,y_slice=None,\
               extend_to_zero=False,extend_to_zero_y=False,x_label=None,y_label=None,z_label=None,title='',show_title=True,\
               show_color_bar=True,color_map='magma',color_map_custom=None,fname=None,z_extrema=None,z_extrema_std=None,\
               invert_y=True,v_line_pos=None,v_line_color='black',show_contours=True,show_contour_zero=True,\
               contour_color=('k','k','w'),contour_linestyles=('-',':'),contour_linewidths=(6,4),\
               show_contour_labels=False,contour_label_format='%1.0f',contour_label_spacing=8,\
               auto_contour=False,custom_contours=None,label_all_custom_contours=False,contour_color_adjust_factor=1,\
               manual_contour_labels=None,axis_line_width=4,show_plot_in_spyder=True,plot_scale=1,\
               font_settings={'weight':'bold','size':22},extend_color_bar='both',z_format=None,\
               ...
               smooth=False,smooth_knot_spacing=(5,5),smooth_concentrated_knots=0,smooth_knot_location=None,smooth_order=(3,3),smooth_exclude_rows=0,\
               blur=False,blur_sigma=None,blur_exclude_rows=0,blur_excluded_sigma=0,rough=None):
...
#    color_map.set_bad(color_map.colors[0])
    if x is None or y is None or z is None:
        if x is None and y is None and z is not None:
            ...
            raise TypeError("color_plot() missing required arguments: 'x','y', and/or 'z'. You must supply 'z' only or all three.")
...
#        plot_norm = matplotlib.colors.LogNorm()#(vmin=1e-6,vmax=1,clip=True)
    ...
    if smooth:
        ...
        def make_knots(a,l,d,s):
            if l==0:
                return np.concatenate((a[0,0]*np.ones(3),a[:d,0],a[d+1:-1:s,0],a[-1,0]*np.ones(4)))
            else:
                return np.concatenate((a[0,0]*np.ones(3),a[0,:l-1-d:s],a[0,l-1-d:l+d-1],a[0,l+d:-1:s],a[0,-1]*np.ones(4)))
        ...
        splfit = bisplrep(x/xscale,y/yscale,z/zscale,kx=smooth_order[0],ky=smooth_order[1],task=-1,tx=tx/xscale,ty=ty/yscale)
    ...
    if show_contours and not auto_contour:
        ...
        if z_extrema is None:
            ...
            if show_contour_zero:
                cs = ax.contour(x,y,z,[level_center],colors=contour_color[0],linestyles=contour_linestyles[0],linewidths=contour_linewidths[0])
                css.append(cs)
            cs = ax.contour(x,y,z,[level_high, level_max],colors=contour_color[1],linestyles=contour_linestyles[1],linewidths=contour_linewidths[1])
            css.append(cs)
            cs = ax.contour(x,y,z,[level_min, level_low],colors=contour_color[2],linestyles=contour_linestyles[1],linewidths=contour_linewidths[1])
            css.append(cs)
...
#            css.append(cs)
                    ...
                    if custom_contour == 0:
                        if show_contour_zero:
                            if 0 == (z_extrema[0]+z_extrema[1])/2:
                                ...
                                cs = ax.contour(x,y,z,[0],colors=contour_color[1],linestyles=contour_linestyles[0],linewidths=contour_linewidths[0])
                            else:
                                cs = ax.contour(x,y,z,[0],colors=contour_color[2],linestyles=contour_linestyles[0],linewidths=contour_linewidths[0])
                            css.append(cs)
                    elif custom_contour >= contour_color_adjust_factor*(z_extrema[0]+z_extrema[1])/2:
                        cs = ax.contour(x,y,z,[custom_contour],colors=contour_color[1],linestyles=contour_linestyles[1],linewidths=contour_linewidths[1])
                        if show_contour_labels:
                            if manual_contour_labels is None:
                                ax.clabel(cs,colors=contour_color[1],fmt=contour_label_format,inline_spacing=contour_label_spacing)
                            else:
                                ax.clabel(cs,colors=contour_color[1],fmt=contour_label_format,inline_spacing=contour_label_spacing,manual=manual_contour_labels[ccidx])
                        css.append(cs)
                    else:
                        cs = ax.contour(x,y,z,[custom_contour],colors=contour_color[2],linestyles=contour_linestyles[1],linewidths=contour_linewidths[1])
                        if show_contour_labels:
                            if manual_contour_labels is None:
                                ax.clabel(cs,colors=contour_color[2],fmt=contour_label_format,inline_spacing=contour_label_spacing)
                            else:
                                ax.clabel(cs,colors=contour_color[2],fmt=contour_label_format,inline_spacing=contour_label_spacing,manual=manual_contour_labels[ccidx])
                        css.append(cs)
    if show_color_bar and not auto_contour:
        if z_label is None:
            ...
            if show_contours and custom_contours is not None:
                cbar = fig.colorbar(im, ax=ax, extend=extend_color_bar, label=z_label, format=z_format, ticks=custom_contours)
            else:
        ...
        if z_extrema is None and show_contours:
            ...
            if custom_contours is None:
                ...
                level_low = (level_min+level_center)/2
                level_high = (level_max+level_center)/2
                cbar.ax.plot([-1e9,1e9],[level_min,level_min],color=contour_color[2],linewidth=contour_linewidths[1],dashes=[1,1])
                cbar.ax.plot([-1e9,1e9],[level_low,level_low],color=contour_color[2],linewidth=contour_linewidths[1],dashes=[1,1])
                cbar.ax.plot([-1e9,1e9],[level_high,level_high],color=contour_color[1],linewidth=contour_linewidths[1],dashes=[1,1])
                cbar.ax.plot([-1e9,1e9],[level_max,level_max],color=contour_color[1],linewidth=contour_linewidths[1],dashes=[1,1])
                cbar.ax.plot([-1e9,1e9],[level_center,level_center],color=contour_color[0],linewidth=contour_linewidths[0])
                if show_contour_zero:
                    cbar.ax.hlines(0,-1,1,colors=contour_color[0],linewidth=contour_linewidths[0]/2,linestyles=contour_linestyles[0])
            else:
                for custom_contour in custom_contours:
                    if label_all_custom_contours or (custom_contour>np.min(z) and custom_contour<np.max(z)):
                        if custom_contour == 0:
                            if show_contour_zero:
                                if 0 == (z_extrema[0]+z_extrema[1])/2:
                                    cbar.ax.plot([-1e9,1e9],[0,0],color=contour_color[0],linewidth=contour_linewidths[0])
                                elif 0 >= contour_color_adjust_factor*(z_extrema[0]+z_extrema[1])/2:
                                    cbar.ax.plot([-1e9,1e9],[0,0],color=contour_color[1],linewidth=contour_linewidths[0])
                                else:
                                    cbar.ax.plot([-1e9,1e9],[0,0],color=contour_color[2],linewidth=contour_linewidths[0])
                        elif custom_contour >= contour_color_adjust_factor*(z_extrema[0]+z_extrema[1])/2:
                            cbar.ax.plot([-1e9,1e9],[custom_contour,custom_contour],color=contour_color[1],linewidth=contour_linewidths[1],dashes=[1,1])
                        else:
                            cbar.ax.plot([-1e9,1e9],[custom_contour,custom_contour],color=contour_color[2],linewidth=contour_linewidths[1],dashes=[1,1])
    elif show_color_bar:
        if z_label is None:
            cbar = fig.colorbar(im, ax=ax, extend=extend_color_bar, format=z_format)
        else:
...
def line_plot(x:ArrayLike,y:ArrayLike,legend=None,x_lim:tuple[float,float]=None,y_lim:tuple[float,float]=None,fig_size:tuple[float,float]=(12,9),\
               ...
               axis_line_width:float=4,plot_scale:float=1,show_plot_in_spyder:bool=True,save_folder:Optional[str]=None,\
               font_settings:dict={'weight':'bold','size':22},y_format=None,x_ticks:Optional[ArrayLike]=None,y_ticks:Optional[ArrayLike]=None,\
               v_line_pos:Optional[ArrayLike]=None,v_line_color='k',vlinewidth:float=6,y_norm:Literal[None,'global','each']=None):
        ...
        if can_iter(v_line_pos): #multiple vlines?
            if can_iter(v_line_pos[0]): #multiple sets of vlines?
                if can_iter(v_line_color) and not isinstance(v_line_color,str): #multiple colors?
                    for vlps, vlc, vlw in zip(v_line_pos,v_line_color,vlinewidth):
                        for vlp in vlps:
                            plt.axvline(x=vlp,color=vlc,linewidth=vlw)
                else: #one color
                    for vlps in v_line_pos:
                        for vlp in vlps:
                            plt.axvline(x=vlp,color=v_line_color,linewidth=vlinewidth)
            else: #one set of vlines
                if can_iter(v_line_color) and not isinstance(v_line_color, str): #multiple colors?
                    for vlp, vlc in zip(v_line_pos,v_line_color):
                        plt.axvline(x=vlp,color=vlc,linewidth=vlinewidth)
                else: #one color
                    for vlp in v_line_pos:
                        plt.axvline(x=vlp,color=v_line_color,linewidth=vlinewidth)
        else: #one vline
            plt.axvline(x=v_line_pos,color=v_line_color,linewidth=vlinewidth)
    ...
    if legend is not None:
        if legend_location == 'outside':
            leg = plt.legend(linelist,legend,title=legend_title,bbox_to_anchor=(1.04,1),loc='upper left',framealpha=1,fancybox=False)
        else:
...
def cylinder_plot(R,Z,T,c,tlims=(0,180),rlims=(0,1),zlims=(0,1),clims=(-2,2),\
    ...
    font_settings={'weight':'bold','size':24},c_modify_exponent=True,c_map="magma",c_map_custom=None,\
    r_interpolation=None,z_interpolation=None,c_ticks=None):
    
    if z_interpolation is not None:
        newZ = np.zeros(((np.shape(Z)[0]-1)*z_interpolation+1,np.shape(Z)[1]))
        newc = np.zeros(((np.shape(c)[0]-1)*z_interpolation+1,)+np.shape(c)[1:])
        newR = np.stack([R[0]]*((np.shape(R)[0]-1)*z_interpolation+1))
        for i in range(np.shape(Z)[0]-1):
            newZ[i*z_interpolation] = Z[i]
            newc[i*z_interpolation] = c[i]
            for k in range(z_interpolation):
                newZ[i*z_interpolation+k+1] = Z[i]*((z_interpolation-k-1)/z_interpolation)+Z[i+1]*((k+1)/z_interpolation) 
                newc[i*z_interpolation+k+1] = c[i]*((z_interpolation-k-1)/z_interpolation)+c[i+1]*((k+1)/z_interpolation) 
        newZ[-1] = Z[-1]
        newc[-1] = c[-1]
    ...
    if r_interpolation is not None:
        newR = np.zeros((np.shape(Z)[0],(np.shape(R)[1]-1)*r_interpolation+1))
        newc = np.zeros((np.shape(c)[0],(np.shape(c)[1]-1)*r_interpolation+1,np.shape(c)[2]))
        newZ = np.stack([Z[:,0]]*((np.shape(Z)[1]-1)*r_interpolation+1),axis=1)
        for j in range(np.shape(R)[1]-1):
            newR[:,j*r_interpolation] = R[:,j]
            newc[:,j*r_interpolation] = c[:,j]
            for k in range(r_interpolation):
                newR[:,j*r_interpolation+k+1] = R[:,j]*((r_interpolation-k-1)/r_interpolation)+R[:,j+1]*((k+1)/r_interpolation) 
                newc[:,j*r_interpolation+k+1] = c[:,j]*((r_interpolation-k-1)/r_interpolation)+c[:,j+1]*((k+1)/r_interpolation) 
        newR[:,-1] = R[:,-1]
        newc[:,-1] = c[:,-1]
    ...
    lowt_real = T[lowidxt]
    hight_real = T[highidxt]
    lowr_real = R[0,lowidxr]
        ...
        for interpolate in np.linspace(0,1,interpolation_amount,endpoint=False):
            ...
            for ridx in [lowidxr,highidxr]:
                ...
                if not (interpolate == 0 and (i == lowidxt or i == highidxt)):
                    tmpplotc = (1-interpolate)*np.ravel(c[lowidxz+1:highidxz,ridx,i])+interpolate*np.ravel(c[lowidxz+1:highidxz,ridx,i+1])
            ...
            for zidx in [lowidxz,highidxz]:
                #don't plot for high/low angle edges
                if not (interpolate == 0 and (i == lowidxt or i == highidxt)):
                    tmpplotc = (1-interpolate)*np.ravel(c[zidx,lowidxr+1:highidxr,i])+interpolate*np.ravel(c[zidx,lowidxr+1:highidxr,i+1])
    ...
    for i in range(lowidxt,highidxt,1):
        for interpolate in np.linspace(0,1,interpolation_amount,endpoint=False):
            ridxlist = [lowidxr+1,highidxr-1] if interpolate == 0 and (i == lowidxt or i == highidxt) else [lowidxr,highidxr]
            for ridx in ridxlist:
                zidxlow = lowidxz+1
                zidxhigh = highidxz
                tmpplotc = (1-interpolate)*np.ravel(c[zidxlow:zidxhigh,ridx,i])+interpolate*np.ravel(c[zidxlow:zidxhigh,ridx,i+1])
    ...
    scatter = ax.scatter(plotx,ploty,plotz,c=plotc,s=matplotlib.rcParams['lines.markersize']**2*markersize**2,edgecolor="none",alpha=1,marker=",",cmap=c_map,linewidth=0,norm=norm,plotnonfinite=True)
    ...
    if show_color_bar:
        if clabel is None:
            ...
            cbar = fig.colorbar(scatter, ax=ax, extend=extend_color_bar, label=clabel, format=c_format, shrink=0.55, pad=0.11)
