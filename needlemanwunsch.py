...
def needleman_wunsch(x:NDArray,y:NDArray,penalty_order:float=1,gap:float=1,insert:float=-1,nw_normalized:bool=False):
    ...
    penalty_order : float, optional
        Order of distance penalty. If 1, the penalty is the difference between two numbers, If 2, the penalty is the square difference, etc. The default is 1.
    gap : float, optional
        The penalty for not matching is `gap**penalty_order`. `gap` corresponds the maximum allowed difference between matched values. The default is 1.
    ...
    for i in range(nx):
        for j in range(ny):
            t[0] = F[i,j] - (2*abs(x[i]-y[j])/(x[i]+y[j]))**penalty_order if nw_normalized else F[i,j] - abs(x[i]-y[j])**penalty_order
...
@njit
def find_matches(x:NDArray,y:NDArray,max_stretch:float=0.02,num_stretches:int=1000,stretching_iterations:int=5,stretch_iteration_factor:float=5,\
    ...
    Tests many stretches, determines the best overall matching quality, and reports the matches and stretch of the best fit.
    ...
    Note: The final precision in the stretching factor is approximately `2*max_stretch/(stretch_iteration_factor**(stretching_iterations-1)*num_stretches)`

    Returns
    -------
    bestrx : NDArray
        Array made up of `x`, with `insert` inserted where there is no match with `y` corresponding to the best quality match.
    bestry : NDArray
        Array made up of `y`, with `insert` inserted where there is no match with `x` corresponding to the best quality match.
    bestq : float
    ...
    """
    bestq = -100000000
    bestrx, bestry, bestq = needleman_wunsch(x,y,penalty_order=penalty_order,gap=gap,insert=insert,nw_normalized=nw_normalized)
    best_stretch = 1
    ...
    for stretching_iteration in range(stretching_iterations):
        if stretching_iteration == 0:
            ...
            search_space = np.linspace(best_stretch-(num_stretches/stretch_iteration_factor/2)*search_space_delta,best_stretch+(num_stretches/stretch_iteration_factor/2)*search_space_delta,num_stretches+2)
        ...
        for i,s in enumerate(search_space):
            rx, ry, q = needleman_wunsch(x,s*y,penalty_order=penalty_order,gap=gap,insert=insert,nw_normalized=nw_normalized)
...
if __name__ == '__main__':
    ...
    kwargs_find_matches = {'max_stretch':max_stretch,'num_stretches':num_stretches,'stretching_iterations':stretching_iterations,\
                            'stretch_iteration_factor':stretch_iteration_factor,'penalty_order':penalty_order,'gap':max_mismatch/2,'nw_normalized':True}
